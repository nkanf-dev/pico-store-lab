...
test('Virtual Desktop and sign-in guides are available in both languages', () => {
  for (const prefix of ['', 'en/']) {
    ...
    const downloads = readFileSync(new URL('download/index.html', root), 'utf8');
    ...
    assert.match(app, /class="adapted-download-notice"/);
    assert.match(app, /href="#downloader"/);
    assert.match(downloads, /client-adaptation-note/);
    assert.doesNotMatch(vrchat, /class="adapted-download-notice"/);
